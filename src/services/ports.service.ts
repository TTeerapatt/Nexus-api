...
const PORT_SELECT = `
  id, port_number, project_name, description, is_active, created_at, updated_at
`;
...
export async function getActivePorts(
  ...
  if (filter.port_number !== undefined) {
    ...
    conditions.push(`port_number = $${params.length}`);
  }
...
export async function createPort(input: CreatePortInput): Promise<PortListItem> {
  ...
  if (!projectName) {
    throw new PortError(400, "project_name is required");
  }
  ...
  try {
    ...
    const inserted = await client.query<PortListItem>(
      `
        INSERT INTO ports (port_number, project_name, description, is_active)
        VALUES ($1, $2, $3, $4)
        RETURNING ${PORT_SELECT}
      `,
      [portNumber, projectName, description, isActive]
    );
...
export async function updatePort(
  ...
  try {
    ...
    const updated = await client.query<PortListItem>(
      `
        UPDATE ports
        SET port_number = $1,
            project_name = $2,
        ...
        RETURNING ${PORT_SELECT}
      `,
      [nextPortNumber, nextProjectName, nextDescription, nextIsActive, id]
    );
    ...
    await client.query("ROLLBACK");
    if (isUniqueViolation(error)) {
      throw new PortError(409, `Port ${nextPortNumber} already exists`);
    }
    throw error;
  } finally {
    client.release();
